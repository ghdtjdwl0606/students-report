      ...
      <div className={`${isMobile ? 'h-[350px]' : 'h-[250px]'} w-full`}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart 
            ...
            margin={isMobile ? { top: 20, right: 10, left: -20, bottom: 20 } : { top: 5, right: 40, left: 20, bottom: 5 }}
          >
            ...
            <Bar dataKey="percentage" radius={isMobile ? [6, 6, 0, 0] : [0, 6, 6, 0]} barSize={isMobile ? 20 : 24}>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
      ...
      {!isShared && (
        <div className="flex flex-wrap justify-end gap-3 no-print px-4 md:px-0">
          <button 
            ...
            className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-white text-slate-700 border border-slate-200 hover:bg-slate-50 px-5 py-3 rounded-xl font-bold transition-all shadow-sm active:scale-95"
          >
            <i className="fas fa-share-nodes text-indigo-500"></i> <span className="hidden sm:inline">학생용</span> 링크 복사
          </button>
          <button 
            onClick={downloadPdf}
            disabled={isGeneratingPdf}
            className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-indigo-600 text-white hover:bg-indigo-700 px-5 py-3 rounded-xl font-bold transition-all shadow-md active:scale-95 disabled:opacity-50"
          >
            {isGeneratingPdf ? (
              <><i className="fas fa-spinner animate-spin"></i> 생성 중</>
            ) : (
              <><i className="fas fa-file-export"></i> PDF 다운로드</>
            )}
          </button>
        </div>
      )}

      <div ref={reportRef} className="space-y-6 p-4 md:p-0 bg-transparent">
        {/* Summary Header */}
        <div className="bg-gradient-to-br from-indigo-700 via-indigo-600 to-violet-700 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 text-white shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6 md:gap-8">
            <div className="text-center md:text-left">
              <div className="inline-block bg-white/20 backdrop-blur-sm px-4 py-1.5 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-widest mb-4">
                Student Score Report
              </div>
              <h2 className="text-3xl md:text-4xl font-black">{result.studentName} 학생</h2>
              <div className="mt-4 flex flex-wrap justify-center md:justify-start gap-3 md:gap-4">
                <div className="bg-white/10 backdrop-blur-md px-3 md:px-4 py-2 rounded-xl border border-white/10">
                  <span className="text-[9px] md:text-[10px] uppercase font-bold opacity-60 block">Reading</span>
                  <span className="text-lg md:text-xl font-bold">{result.scoreR} / 160</span>
                </div>
                <div className="bg-white/10 backdrop-blur-md px-3 md:px-4 py-2 rounded-xl border border-white/10">
                  <span className="text-[9px] md:text-[10px] uppercase font-bold opacity-60 block">Listening</span>
                  <span className="text-lg md:text-xl font-bold">{result.scoreL} / 160</span>
                </div>
              </div>
            </div>
            <div className="bg-white text-slate-900 rounded-3xl p-6 md:p-8 text-center shadow-xl border border-white/20 min-w-full md:min-w-[240px]">
              <span className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest">Final Total Score</span>
              <div className="text-5xl md:text-6xl font-black text-indigo-600 mt-1 md:mt-2">
                {result.totalScore}
              </div>
              <div className="mt-2 text-slate-400 text-xs md:text-sm font-bold border-t border-slate-100 pt-2">
                MAX: {result.maxScore}
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          {/* AI Insight Card */}
          <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-200 shadow-sm flex flex-col overflow-hidden relative transition-all">
      ...
      <div className="flex justify-center pt-8 no-print px-4">
        <button 
          onClick={onReset}
          className="w-full md:w-auto bg-slate-900 hover:bg-black text-white px-12 py-4 rounded-2xl font-bold transition-all shadow-xl shadow-slate-200 active:scale-95"
        >
          {isShared ? "나만의 성적표 직접 만들기" : "새로운 시험 결과 입력하기"}
        </button>
      </div>
    </div>
  );
